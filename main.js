...
ipcMain.handle('run-mulle-command', async (event, command) => {
  ...
  try {
    ...
    const result = execSync(cmd, { encoding: 'utf-8', cwd: currentProjectPath || process.cwd() });
    return result.trim();
  } catch (error) {
...
async function readEnvironmentFiles(projectPath) {
  ...
  const hardcodedScopes = [
    { scope: 'plugin', type: 'share', priority: 5 },
    { scope: `plugin-os-${mulleUname}`, type: 'share', priority: 15 },
    { scope: 'project', type: 'etc', priority: 20 }, // auxscope default
    { scope: 'global', type: 'etc', priority: 40 },
    { scope: `os-${mulleUname}`, type: 'etc', priority: 60 },
    { scope: `host-${mulleHostname}`, type: 'etc', priority: 80 },
    { scope: `user-${mulleUsername}`, type: 'etc', priority: 100 },
    { scope: `user-${mulleUsername}-os-${mulleUname}`, type: 'etc', priority: 120 },
    { scope: `user-${mulleUsername}-host-${mulleHostname}`, type: 'etc', priority: 140 },
    { scope: 'custom', type: 'etc', priority: 1000 },
    { scope: 'post-global', type: 'etc', priority: 2000 }
  ];
