...
const selectedScopes = new Set(); // Track which scopes are selected for filtering
let systemInfo = {
   os      : null,
   hostname: null,
   username: null 
}; // Actual system info
let effectiveSystemInfo = {
   virtualRoot: null,
   os         : null,
   hostname   : null,
   username   : null 
...
const settingsVirtualRoot = document.getElementById("settings-virtual-root");
const settingsOS = document.getElementById("settings-os");
const settingsHostname = document.getElementById("settings-hostname");
...
resetSettings.addEventListener("click", resetSettingsToSystem);
...
{
   console.log("Menu event: Export");
   exportAsShellScript();
});

window.electronAPI.onMenuSettings(() => 
{
   console.log("Menu event: Settings");
   showSettingsModal();
});
...
{
   console.log("Menu event: Close Project");
...
// Preview control event listeners removed
...
async function loadSystemInfo() 
{
   try 
   {
      ...
      // Initialize effective system info with actual values
      effectiveSystemInfo = {
         virtualRoot: currentProjectPath,
         os         : systemInfo.os,
...
async function openProject(projectPath) 
{
   ...
   try 
   {
      ...
      if (result.success) 
      {
         ...
         // Initialize effective system info
         effectiveSystemInfo.virtualRoot = currentProjectPath;
      
         // Add hardcoded system scope variables at priority 0
         await addSystemScopeVariables();
...
async function addSystemScopeVariables() 
{
   // Calculate MULLE_VIRTUAL_ROOT_ID
   const virtualRoot = effectiveSystemInfo.virtualRoot || currentProjectPath || "";
   const virtualRootId = await window.electronAPI.calculateVirtualRootId(virtualRoot);
  
   // Create hardcoded system scope at priority 0 (highest priority)
   const systemVariables = [
      {
         key      : "MULLE_HOSTNAME",
         value    : effectiveSystemInfo.hostname || systemInfo.hostname || "",
         scope    : "system",
         scopeType: "system",
         priority : 0,
         editable : false,
      ...
      {
         key      : "MULLE_USERNAME",
         value    : effectiveSystemInfo.username || systemInfo.username || "",
         scope    : "system",
         scopeType: "system",
         priority : 0,
         editable : false,
      ...
      {
         key      : "MULLE_UNAME",
         value    : effectiveSystemInfo.os || systemInfo.os || "",
         scope    : "system",
         scopeType: "system",
         priority : 0,
         editable : false,
...
function isScopeApplicable(scopeName) 
{
   ...
   const effectiveOS = (effectiveSystemInfo.os || systemInfo.os || "").toLowerCase();
   const effectiveHostname = (effectiveSystemInfo.hostname || systemInfo.hostname || "").toLowerCase();
   const effectiveUsername = (effectiveSystemInfo.username || systemInfo.username || "").toLowerCase();
  
   // Match os-<uname> pattern (followed by - or end of string)
   const osMatch = lowerScope.match(/\bos-([a-z0-9]+)(?:-|$)/);
   if (osMatch && osMatch[1] !== effectiveOS) 
   {
      return false;
   }
   ...
   if (hostMatch && hostMatch[1] !== effectiveHostname) 
   {
      return false;
   }
   ...
   if (userMatch && userMatch[1] !== effectiveUsername) 
   {
      return false;
   }
...
function showScopeManagerForVariable(variableIndex) 
{
   ...
   // Show effective system values
   const effectiveSystemInfoDiv = document.getElementById("effective-system-info");
   const effectiveOS = effectiveSystemInfo.os || systemInfo.os;
   const effectiveHostname = effectiveSystemInfo.hostname || systemInfo.hostname;
   const effectiveUsername = effectiveSystemInfo.username || systemInfo.username;
   
   const hasOverrides = effectiveSystemInfo.os !== systemInfo.os || 
                        effectiveSystemInfo.hostname !== systemInfo.hostname || 
                        effectiveSystemInfo.username !== systemInfo.username;
   
   effectiveSystemInfoDiv.innerHTML = hasOverrides 
      ? `<strong>Effective values (from Settings):</strong> OS: <code>${effectiveOS}</code>, Host: <code>${effectiveHostname}</code>, User: <code>${effectiveUsername}</code>`
      : `<strong>System values:</strong> OS: <code>${effectiveOS}</code>, Host: <code>${effectiveHostname}</code>, User: <code>${effectiveUsername}</code>`;
...
function handleModalDrop(e) 
{
   ...
   if (!draggedScopeInfo || !dropScopeInfo) 
   {
      return false;
   }
...
function updateVariableLive(index, value) 
{
   ...
   if (updateVariableLive.timeout) 
   {
      clearTimeout(updateVariableLive.timeout);
   }
   updateVariableLive.timeout = setTimeout(async () => 
   {
      if (!isScopeApplicable(variable.scope)) 
      {
         previewEvaluated.textContent = "(scope not applicable for current system)";
      ...
      const evaluated = await evaluateValue(value, index);
...
async function selectVariable(index) 
{
   ...
   // Check if this variable's scope is applicable
   if (!isScopeApplicable(variable.scope)) 
   {
      previewEvaluated.textContent = "(scope not applicable for current system)";
      previewEvaluated.style.fontStyle = "italic";
...
async function evaluateValue(value, currentIndex) 
{
   ...
   for (let i = 0; i < currentIndex; i++) 
   {
      ...
      if (v.enabled !== false && isScopeApplicable(v.scope)) 
      {
         env[v.key] = v.value || "";
      }
   }
...
function addVariable() 
{
   ...
   if (actualIndex >= 0) 
   {
      ...
      {
         rows[renderedIndex].scrollIntoView({
            behavior: "smooth",
            block   : "center" 
         });
...
async function exportAsShellScript() 
{
   try 
   {
      const filePath = await window.electronAPI.exportScriptDialog();
      if (!filePath) 
      {
         return;
      }
      ...
      script += "#\n";
      script += "# Exported environment from mulle-environment-editor\n";
      script += `# Date: ${new Date().toISOString()}\n`;
      script += "#\n";
      script += "# To test this script without affecting your current environment:\n";
      script += "#   env -i bash -c '. environment-export.sh && env | sort'\n";
      script += "#\n\n";

      // Add system variable setup (from environment.sh)
      script += "# Setup system variables if not already set\n";
      script += 'if [ -z "${MULLE_VIRTUAL_ROOT}" ]; then\n';
      script += '   MULLE_VIRTUAL_ROOT="`PATH=/bin:/usr/bin pwd -P`"\n';
      script += '   echo "Using ${MULLE_VIRTUAL_ROOT} as MULLE_VIRTUAL_ROOT" >&2\n';
      script += "fi\n\n";
      script += 'if [ -z "${MULLE_UNAME}" ]; then\n';
      script += '   MULLE_UNAME="`PATH=/bin:/usr/bin uname -s 2> /dev/null | tr \'[:upper:]\' \'[:lower:]\'`"\n';
      script += '   MULLE_UNAME="${MULLE_UNAME:-unknown}"\n';
      script += '   echo "Using ${MULLE_UNAME} as MULLE_UNAME" >&2\n';
      script += "fi\n\n";
      script += 'if [ -z "${MULLE_HOSTNAME}" ]; then\n';
      script += '   MULLE_HOSTNAME="`PATH=/bin:/usr/bin hostname -s 2> /dev/null`"\n';
      script += '   MULLE_HOSTNAME="${MULLE_HOSTNAME:-unknown}"\n';
      script += "fi\n\n";
      script += 'if [ -z "${MULLE_USERNAME}" ]; then\n';
      script += '   MULLE_USERNAME="${USER:-${LOGNAME:-unknown}}"\n';
      script += "fi\n\n";
      script += 'if [ -z "${MULLE_VIRTUAL_ROOT_ID}" ]; then\n';
      script += '   MULLE_VIRTUAL_ROOT_ID="$(shasum -a 256 <<< "${MULLE_VIRTUAL_ROOT}")"\n';
      script += '   MULLE_VIRTUAL_ROOT_ID="${MULLE_VIRTUAL_ROOT_ID:1:12}"\n';
      script += "fi\n\n";

      // Export all applicable variables in priority order
      ...
      for (const variable of currentEnvData.allVariables) 
      {
         ...
         const scopeInfo = `# [${variable.scopeType}:${variable.scope}, priority: ${variable.priority}]`;
         script += `${scopeInfo}\n`;
         script += `export ${variable.key}="${variable.value || ""}"${comment}\n`;
      }

      showStatus("Exporting shell script...", "info");
      const result = await window.electronAPI.exportShellScript(script, filePath);

      if (result.success) 
      {
         showStatus("Shell script exported successfully", "success");
      }
      else 
      {
         showStatus(`Failed to export: ${result.error}`, "error");
      }
   }
   catch (error) 
   {
...
{
   // Populate with current effective values (show overrides if any)
   settingsVirtualRoot.value = effectiveSystemInfo.virtualRoot !== currentProjectPath ? effectiveSystemInfo.virtualRoot : "";
   settingsVirtualRoot.placeholder = currentProjectPath || "Current project path";
   settingsOS.value = effectiveSystemInfo.os !== systemInfo.os ? effectiveSystemInfo.os : "";
   settingsOS.placeholder = systemInfo.os || "Current OS";
   settingsHostname.value = effectiveSystemInfo.hostname !== systemInfo.hostname ? effectiveSystemInfo.hostname : "";
   settingsHostname.placeholder = systemInfo.hostname || "Current hostname";
   settingsUsername.value = effectiveSystemInfo.username !== systemInfo.username ? effectiveSystemInfo.username : "";
   settingsUsername.placeholder = systemInfo.username || "Current username";
...
{
   // Update effective system info with overrides (or fall back to actual values)
   effectiveSystemInfo.virtualRoot = settingsVirtualRoot.value.trim() || currentProjectPath;
   effectiveSystemInfo.os = settingsOS.value.trim() || systemInfo.os;
   effectiveSystemInfo.hostname = settingsHostname.value.trim() || systemInfo.hostname;
   effectiveSystemInfo.username = settingsUsername.value.trim() || systemInfo.username;
  
   // Regenerate system variables with new values
   await addSystemScopeVariables();
  
   // Re-render to update scope visibility
   renderVariables();
...
function resetSettingsToSystem() 
{
   // Clear all overrides
   effectiveSystemInfo.virtualRoot = currentProjectPath;
   effectiveSystemInfo.os = systemInfo.os;
   effectiveSystemInfo.hostname = systemInfo.hostname;
   effectiveSystemInfo.username = systemInfo.username;
  
   // Clear input fields
   settingsVirtualRoot.value = "";
   settingsOS.value = "";
   settingsHostname.value = "";
